...
import {setUserToFileAC} from "../../Reducers/files/newFileReducer";
...
const TreeNode = (props) => {
  ...
  let isClickedRow = useSelector((state => state.newFile.isClickedRow))
  let isCallRow = useSelector((state => state.newFile.isCallRow))
  ...
  const setChosen = () => {
    ...
    props.handleSetNodeValue && props.handleSetNodeValue({
        targetName: props.node.firstName + ' ' + props.node.lastName,
...
//departamentis saxelis  shecvla
  ...
  let onUserClick = () => {
    if (props.isSharedUsers === true) {
      // failis gaziarebistvis userebis shercheva struqturashi
    ...
    if (props.isSharedUsers === false) {
      // failis velebis gakonroleba
      if (URL === '/newFile' || '/file' && props.isAppointment === false) {
        const values = [...props.ColRows]
        let addUser = {
          displayName: `${props.node.firstName} ${props.node.lastName},`,
          userId: props.node.userId,
        }
        ...
        if (isCallRow === false) {
          values[asideFileIndex][insideIndex].users.push(addUser)
        } else {
          if (isClickedRow) {
            //failshi marcxena mxares indexsaciaze iuzeris archeva
            values[asideFileIndex].map(item => item.users.push(addUser))
          } else {
            values.map(item => item[asideFileIndex].users.push(addUser))
          }
        }
